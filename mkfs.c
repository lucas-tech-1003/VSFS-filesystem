...
static bool mkfs(void *image, size_t size, mkfs_opts *opts)
{
	...
	// calculate number of blocks needed for inode table
	u_int32_t num_blks_itable =  div_round_up(opts->n_inodes, inodes_per_block);
	// mark inode table blocks allocated
	for (u_int32_t i = 0; i < num_blks_itable; i++) {
	...
	// allocate first block after inode table for root dir data block
	bitmap_set(dbmap, nblks, VSFS_ITBL_BLKNUM + num_blks_itable, true);
	root_ino->i_direct[0] = VSFS_ITBL_BLKNUM + num_blks_itable;

	// 4. Create '.' and '..' entries in root dir data block.
	root_entries = (vsfs_dentry *)(image + (VSFS_ITBL_BLKNUM + num_blks_itable) * VSFS_BLOCK_SIZE);
	root_entries[0].ino = VSFS_ROOT_INO; // '.' entry points to itself
	root_entries[1].ino = VSFS_ROOT_INO; // '..' entry points to itself
	...
	// Set the .ino field to VSFS_INO_MAX for directory entries in a block 
	// that are not in use (either because the block was just allocated to the directory, 
	// 							or because a file was deleted)
	uint32_t num_entries_per_block = VSFS_BLOCK_SIZE / sizeof(vsfs_dentry);
	for (uint32_t i = 2; i < num_entries_per_block; i++) {
	...
	sb->sb_free_inodes = opts->n_inodes - 1; // since root dir inode occupies 1
	sb->sb_num_blocks = nblks;
	sb->sb_free_blocks = nblks - (VSFS_ITBL_BLKNUM + num_blks_itable) - 1; // NEED FIX since root dir data block occupies 1
	sb->sb_data_region = VSFS_ITBL_BLKNUM + num_blks_itable; // first block after inode table
